...
class _PreferencesState extends State<Preferences> {
  ...
  void initSearchPreferences() async {
    ...
    setState(() {
      _animalType = AnimalType.values[userDoc.data['prefsAnimalType']] ?? AnimalType.none;
      _catBreeds = CatBreeds.values[userDoc.data['prefsCatBreeds']] ?? CatBreeds.none;
      _dogBreeds = DogBreeds.values[userDoc.data['prefsDogBreeds']] ?? DogBreeds.none;
      _otherBreeds = OtherBreeds.values[userDoc.data['prefsOtherBreeds']] ?? OtherBreeds.none;
  ...
  Widget breed() {
    return Text('Breed', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16));
  }

  Widget animalBreed(BuildContext context) {
    if (_animalType == AnimalType.cat) { return catBreeds(context); }
    else if (_animalType == AnimalType.dog) { return dogBreeds(context); }
    else if (_animalType == AnimalType.other) { return otherBreeds(context); }
    else {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
  ...
  Widget animalBehavior(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Text('Behavior Traits', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        InkWell(
          onTap: () {
            setState(() {
